...
export default function EditTaskModal({ task, onClose }) {
  ...
  /* Pre-fill with the task being edited */
  useEffect(() => {
    if (task) {
  ...
  function handleSubmit(e) {
    ...
    dispatch(editTask({ id: task.id, ...form }))
    onClose()
  }
      ...
      <div className="bg-white w-full max-w-md rounded-2xl overflow-hidden
        ...
        <div className="px-6 pt-6 pb-5 flex items-start gap-4 border-b border-slate-100">
          ...
          <div className="flex-1 min-w-0">
            ...
            {/* Show the original title as breadcrumb */}
            <p className="text-xs text-slate-400 mt-0.5 truncate" title={task?.title}>
              {task?.title}
            </p>
          </div>
